...
# dataset settings
dataset_type = 'HandSlideDatasetInputReturn'
train_pipeline = [
    dict(type='LandmarkAddNoise'),
    dict(type='LandmarkNormalize'),
    dict(type='LandmarkSequencePadding', max_duration=10),
    dict(type='LandmarkToTensor', keys=['img']),
    dict(type='ToTensor', keys=['gt_label']),
    dict(type='Collect', keys=['img', 'gt_label'], meta_keys=['duration', 'landmark', 'label', 'src_depth_paths'])
]
test_pipeline = [
    dict(type='LandmarkNormalize'),
    dict(type='LandmarkSequencePadding', max_duration=10),
    dict(type='LandmarkToTensor', keys=['img']),
    dict(type='Collect', keys=['img'], meta_keys=['duration', 'landmark', 'label', 'src_depth_paths'])
]
...
data = dict(
    ...
    train=gen_sub_data("/data/dataset/hand/backup/slide/website_simulation/2023_01_10/", False),
    # val=gen_all_slide("/data/dataset/hand/backup/slide/website_simulation/2023_01_09/", True),
    # test=gen_all_slide("/data/dataset/hand/backup/slide/website_simulation/2023_01_09/", True),
    val=gen_sub_data("/data/dataset/hand/backup/slide/website_simulation/2023_01_11/", True),
    test=gen_sub_data("/data/dataset/hand/backup/slide/website_simulation/2023_01_11/", True),
        )
# optimizer
